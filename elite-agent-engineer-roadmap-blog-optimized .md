---
title: "精英 Agent 工程师学习路线：从范式理解到生产级落地"
published: 2026-03-20
description: "一份面向 Agent 工程师成长的系统化路线：覆盖 Prompt、Harness、Tool Calling、MCP、Memory、RAG、Multi-Agent、评估、AgentOps 与生产维护。"
tags: [Agent, AI工程, LangGraph, RAG, Multi-Agent, AgentOps, 学习路线, featured]
category: Agent
draft: false
toc: true
---
...
> **定位**：从理论范式到工程落地，建立面向生产级 Agent 系统的完整能力闭环。  
> **适合人群**：想从普通 AI 应用开发进阶到 Agent 架构设计、工程实现、评估优化与长期维护的开发者。  
> **推荐主线项目**：`OrderFlow-Agent` —— 面向电商交易履约场景的可验证多 Agent 决策系统。
...
| 模块 | 你将掌握什么 | 结果 |
|---|---|---|
| 基础能力 | Python 工程化、FastAPI、Pydantic、Docker、Redis、PostgreSQL | 能搭建 Agent 后端工程 |
| 核心范式 | ReAct、Plan-and-Execute、Workflow、Agentic RAG、Multi-Agent | 能判断业务场景适合哪种范式 |
| Harness 架构 | Context、Memory、Tools、Planner、Executor、Guardrails、Evaluator | 能设计完整 Agent 系统 |
| 工程落地 | Tool Calling、MCP、Agent Skills、LangGraph、RAG、AgentOps | 能实现可运行、可维护系统 |
| 评估优化 | Golden Dataset、Trajectory Eval、Failure Taxonomy、Eval as CI Gate | 能量化优化 Agent 效果 |
| 生产维护 | 观测、回放、灰度、回归测试、成本控制、降级策略 | 能把 Demo 变成生产级系统 |
...
主线 E：求职与长期壁垒
README → 技术博客 → 评估报告 → 简历项目 → 面试表达 → 技术沉淀/开源维护
```
...
| 阶段 | 主题 | 核心目标 |
|---|---|---|
| 阶段 0 | 前置基础补齐 | 搭建 Agent 工程底座 |
| 阶段 1 | LLM 与 Prompt Engineering | 把 Prompt 做成可测试资产 |
| 阶段 2 | Agent 基础范式 | 掌握范式边界与业务适配 |
| 阶段 3 | Agent Harness 架构 | 建立完整系统模块认知 |
| 阶段 4 | 工具调用、MCP 与 Agent Skills | 连接真实业务系统 |
| 阶段 5 | Memory 与 Context Engineering | 管理上下文与长期记忆 |
| 阶段 6 | Agentic RAG / GraphRAG / RuleRAG | 构建可追溯业务证据链 |
| 阶段 7 | Planning / Reflection / Verifier | 处理复杂任务与自我修正 |
| 阶段 8 | Multi-Agent 系统 | 设计多角色协同机制 |
| 阶段 9 | Agent 微调工程 | 判断何时需要微调以及如何构造轨迹数据 |
| 阶段 10 | Agent 安全、约束与可靠性 | 构建安全边界与防护机制 |
| 阶段 11 | 评估、测试集与可观测性 | 建立量化评估和失败归因体系 |
| 阶段 12 | 生产部署、AgentOps 与长期维护 | 从 Demo 走向生产级系统 |
| 阶段 13 | 业务场景落地训练 | 把能力迁移到真实业务 |
| 阶段 14 | 学习节奏与时间规划 | 制定 3/6/12 个月成长计划 |
| 阶段 15 | 推荐资料清单 | 建立长期资料库 |
| 阶段 16 | 最终能力检查清单 | 自测是否具备 Agent 工程能力 |
...
> 这是最适合作为长期主项目的场景，因为它同时覆盖工具调用、规则 RAG、状态校验、安全约束和端到端评估。
